...
export default function useVerticalHydroReal(cfg) {
  const C = useMemo(() => ({
    ...
    targetSpeed: cfg.targetSpeed ?? 1.0,
    verticalSpeed: cfg.verticalSpeed ?? 0.5,
    lerpFactor: cfg.lerpFactor ?? 2.0,
  }), [cfg])
  ...
  const api = {
    ...
    step(dt, forward = new THREE.Vector3(0, 0, -1), right = new THREE.Vector3(1, 0, 0)) {
      ...
      if (keys.current.w) { desiredVx += forward.x * C.targetSpeed; desiredVz += forward.z * C.targetSpeed }
      if (keys.current.s) { desiredVx -= forward.x * C.targetSpeed; desiredVz -= forward.z * C.targetSpeed }
      if (keys.current.d) { desiredVx += right.x * C.targetSpeed; desiredVz += right.z * C.targetSpeed }
      if (keys.current.a) { desiredVx -= right.x * C.targetSpeed; desiredVz -= right.z * C.targetSpeed }
